...
(function () {
  ...
  function validate() {
    ...
    const teen = form.teen_name;
    const age = form.teen_age;
    const night = form.class_night;
    ...
    if (!teen.value.trim()) { showError(teen, "Please enter your teen's first name."); ok = false; }
    else clearError(teen);

    if (!age.value) { showError(age, "Please pick your teen's age."); ok = false; }
    else clearError(age);

    if (!night.value) { showError(night, 'Please pick a preferred class night.'); ok = false; }
    else clearError(night);

    return ok;
  }

  // Clear a field's error as soon as the parent fixes it.
  ['parent_name', 'phone', 'email', 'teen_name', 'teen_age', 'class_night'].forEach((n) => {
  ...
  document.body.addEventListener('htmx:afterRequest', (e) => {
    ...
    if (e.detail.successful) {
      ...
      // Fire the Meta Lead conversion event, tagged for the teen ad set.
      // (No-op until PIXEL_ID is set in index.html.)
      if (window.fbq) fbq('track', 'Lead', { content_category: 'teens' });
    } else {
  ...
  // -----------------------------------------------------------------
  // Scroll-depth 50% — custom pixel event, fired once.
  // -----------------------------------------------------------------
  ...
  function onScroll() {
    ...
    if (scrolled >= halfway) {
      ...
      if (window.fbq) fbq('trackCustom', 'ScrollDepth50', { content_category: 'teens' });
    }
  }
